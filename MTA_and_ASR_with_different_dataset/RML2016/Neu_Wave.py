...
POISON_FRACTION  = 0.2
BACKDOOR_TARGET  = 9          # target class index in [0..10]
...
def _train_local_model(base_model, dataloader, epochs, lr, device,
    ...
    """
    从 base_model 出发做本地训练，返回训练后的完整参数向量 local_vec。
    poison=True 时，对部分 batch 加 IQ trigger 并改成目标标签。
    """
...
def neurotoxin_combine(delta_benign: torch.Tensor,
                       ...
                       eps: float = 1e-12):
    """
    程序1同款 Neurotoxin-style 组合：
    1) 用 benign delta 的低贡献维度定义 LCD
    2) 在 LCD 上用 poisoned delta 替换 benign delta
    3) 再做一次整体插值并放大
    """
...
# ============================================================
# 4) Local train (Neurotoxin attack for malicious clients)
# ============================================================
def local_train(model, global_model, dataloader, epochs, lr, device,
                malicious=False, poison_frac=0.0, backdoor_target=0,
                trig_len=16, i_val=3.0, q_val=-3.0, trig_pos="tail",
                nt_lcd_frac=0.7, nt_alpha=1.0, nt_scale_attack=1.5):
    """
    benign client:
        正常训练，返回 benign delta
    malicious client:
        先训练 benign delta，再训练 poisoned delta，
        最后用程序1同款 neurotoxin_combine 得到 delta_attack
    """
