...
Model: TextCNN
Attack: backdoor by inserting trigger token (replace last non-pad token)
"""
...
DIRICHLET_ALPHA  = 0.5
POISON_FRACTION  = 0.2
BACKDOOR_TARGET  = 3        # AG News labels are 0..3
...
def _train_local_model(base_model, dataloader, epochs, lr, device,
    ...
    """
    从 base_model 出发做本地训练，返回训练后的完整参数向量。
    poison=True 时，对部分 batch 注入 trigger token 并改成目标标签。
    """
...
def neurotoxin_combine(delta_benign: torch.Tensor,
                       ...
                       eps: float = 1e-12):
    """
    与程序1一致的 Neurotoxin-style 组合：
    1) 按 benign delta 的低贡献维度定义 LCD
...
def local_train(model, global_model, dataloader, epochs, lr, device,
    ...
    malicious client:
        ...
        最后用 Neurotoxin 组合得到 delta_attack
    """
...
# NOTE: one_round里调用时请加 model=global_model
