...
def local_train(model, global_model, dataloader, epochs, lr, device,
    ...
    delta_attack = neurotoxin_combine(delta_benign, delta_poison,
                                      lcd_frac=0.9,  # 你已经改过
                                      alpha = 1,  # 新增：向 poisoned 靠拢程度
                                      scale_attack = 1,  # 新增：整体放大系数
                                      eps=1e-12)
...
#     """
#     Weakened whole-model inspection for suspicious clients:
#       1) relaxed whole-model MAD on ||delta_i||_2
#       2) only use 3 coarse whole-model features: [dev, L2, dir_angle]
#       3) no innocent criterion; after AHC, choose the larger cluster as honest
#
#     返回：
#         passed_ids, deny_details
#     """
...
#
#         # 去掉 innocent criterion，直接选人数更多的一簇作为 honest cluster
#         # 若人数相同，则选平均 dev 更小的一簇
...
def ps_inspect_whole_model(suspicious_updates: List[dict],
                           ref_vec: torch.Tensor,
                           log_prefix: str = "[PS-Whole]"):
    """
    Whole-model inspection:
    1) whole-model MAD on suspicious deltas
    2) whole-model feature extraction
    3) AHC split
    4) innocent criterion
    """
    susp_cids = [u["cid"] for u in suspicious_updates]
    K = len(suspicious_updates)
    ...
    glist = [u["delta"].detach().cpu().numpy().astype(np.float32) for u in suspicious_updates]
    ref_np = ref_vec.detach().cpu().numpy().astype(np.float32)

    deny_details = {
        cid: {"mad_fail": False, "ahc_non_honest": False}
        for cid in susp_cids
    }
    ...
    if len(cand1_idx) == 0:
        cand1_idx = list(range(K))
        cand2_idx = []

    choose_cand = cand1_idx

    if len(cand2_idx) > 0:
        winner, _, _ = _innocent_choose(cand1_idx, cand2_idx, glist, ref_np)
        if winner == 2:
            choose_cand = cand2_idx

    for i in range(K):
        if i not in choose_cand:
            deny_details[susp_cids[i]]["mad_fail"] = True

    if len(choose_cand) == 1:
        passed_ids = [susp_cids[choose_cand[0]]]
        denied_ids = [cid for cid in susp_cids if cid not in passed_ids]
        for cid in denied_ids:
            print(f"{log_prefix} cid={cid:2d} DENY | "
                  f"mad_fail={deny_details[cid]['mad_fail']} | "
                  f"ahc_non_honest={deny_details[cid]['ahc_non_honest']}")
        return passed_ids, deny_details
    ...
    for i in choose_cand:
        ...
        Dmean = 0.0 if len(others) == 0 else float(np.mean([np.linalg.norm(g - o) for o in others]))

        dev = float(np.abs(ref_np - g).sum())
        L2 = float(np.linalg.norm(g))
        denom = (np.linalg.norm(g) * np.linalg.norm(ref_np) + 1e-12)
        cosv = np.clip((g @ ref_np) / denom, -1.0, 1.0)
        dir_angle = float(np.arccos(cosv))
    ...
    c1, c2 = _ahc_split(feats)
    winner, _, _ = _innocent_choose(
        [choose_cand[i] for i in c1],
    ...
    passed_ids = [susp_cids[i] for i in honest_idx]

    for i in range(K):
        cid = susp_cids[i]
        if cid not in passed_ids:
            if not deny_details[cid]["mad_fail"]:
                deny_details[cid]["ahc_non_honest"] = True
...
def one_round(round_idx, global_model, client_loaders, malicious_ids,
    ...
    acc_ids, deny_details = ps_inspect_whole_model(suspicious_updates, ref_vec)
...
def simulate_many_rounds(num_rounds=10, split=(0.50,0.30,0.20), verbose_round=True,
    ...
    # 只保留一条防御链（whole-model inspection）
