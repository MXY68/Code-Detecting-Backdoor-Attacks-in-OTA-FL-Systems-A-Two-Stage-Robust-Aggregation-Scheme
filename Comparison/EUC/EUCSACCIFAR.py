...
GROUP_PREV_UPDATES = {}           # gid -> previous round's accepted/observed group update
...
def compute_reference_update(global_model, ref_loader, device, lr=LR, max_batches=1):
    """
    PS 用一小份 clean reference data 做 1~max_batches 个 batch 的 SGD，
...
def compute_group_score(group_update: torch.Tensor,
                        ...
                        eps=1e-12):
    """
    group-level lightweight multi-indicator filtering:
...
def one_round_fedsac_baseline(round_idx,
    ...
    FedSAC-style baseline:
      ...
      3) group aggregation
      4) PS-side group-level multi-indicator filtering
      5) accepted groups aggregate to update global model
      6) accepted groups' members gain reputation
    """
    ...
    # --------------------------------------------------
    # Step 4: group aggregation + group-level scoring
    # --------------------------------------------------
    group_infos = []
    for gid, members in enumerate(groups):
        group_updates = [update_by_cid[cid] for cid in members]
        agg_group, _ = weighted_average(group_updates)   # 组内先沿用样本数加权平均
        ...
        accept = (stat["score"] >= GROUP_SCORE_THRESHOLD)
