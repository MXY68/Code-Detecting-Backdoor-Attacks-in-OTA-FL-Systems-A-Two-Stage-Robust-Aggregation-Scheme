# -*- coding: utf-8 -*-
"""
OTA-FL with PS-side whole-model inspection
Comparison scheme:
client trust split + PS whole-model inspection + RS filtering
"""
...
POISON_FRACTION   = 0.1
BACKDOOR_TARGET   = 0
LAMBDA_EUC        = 0.1   # 参数名保留，但 local_train 内实际是 cosine/norm-constrained malicious training
BENIGN_CLEAN_RATIO = 1.0

RANDOMIZE_MALICIOUS = True
MALICIOUS_SEED = 12345

# RS / warm-up hyperparams
...
def one_round(round_idx, global_model, client_loaders, malicious_ids,
    ...
    if verbose:
        for u in updates:
            ...
            print(f"CID {u['cid']:2d} [{label}] | trust={u['trust']:.3f} | TDA={u['tda']:+.3f} | "
                  f"relL2={u['rel_l2']:.3f} | spiky={u['spiky']:.3f} | "
                  f"temporal={u['temporal']:+.3f} | wsign={u['wsign']:.3f}")
    ...
    # ----- trusted aggregate as reference -----
    ...
    # ----- PS inspection on suspicious via whole-model inspection -----
    acc_ids, deny_details = ps_inspect_whole_model(
        suspicious_updates,
        ref_vec,
        log_prefix="[PS-Whole]"
    )
    ...
    return dict(
        models=dict(clean=clean_global, trusted=trusted_global, final=final_global),
        groups=groups,
