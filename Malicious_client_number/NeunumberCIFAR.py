...
def neurotoxin_combine(delta_benign: torch.Tensor,
                       ...
                       scale_attack: float = 1.5,  # 新增：整体放大系数
                       eps: float = 1e-12):
...
def client_compute_trust_local_only(theta_prev_vec, delta_i_vec, prev_delta_i_vec=None,
    ...
    w_tda, w_rel, w_spk, w_w = 0.15, 0.45, 0.40, 0.0
    trust = (w_tda * tda01 +
             w_rel * rel_score +
             w_spk * spky_score +
             w_w   * wsign_score)
...
if __name__ == "__main__":
    ...
    for num_mal in malicious_num_list:
        ...
        # 每次新实验前清空状态，避免不同 malicious 数量之间互相污染
        client_states.clear()
        RS.clear()
        ...
        # =========================
        # 2) 保存每种 malicious 数量下的汇总
        # =========================
        final_acc_last = LOGS["acc_final"][-1]
        final_asr_last = LOGS["asr_final"][-1]
        avg_acc_last10 = float(np.mean(LOGS["acc_final"][-10:]))
        avg_asr_last10 = float(np.mean(LOGS["asr_final"][-10:]))
        avg_selected_mal_last10 = float(np.mean(LOGS["num_selected_malicious"][-10:]))
        avg_removed_benign_last10 = float(np.mean(LOGS["num_removed_benign"][-10:]))
        ...
        print(f"Last 10 mean | Acc = {avg_acc_last10:.2f}% | ASR = {avg_asr_last10:.2f}%")

    # =========================
    # 3) 保存每轮明细 CSV
    # =========================
    detail_csv_path = "Neunumber0.csv"
    ...
    # 4) 保存汇总 CSV
    # =========================
    summary_csv_path = "Neunumber1.csv"
    with open(summary_csv_path, "w", newline="") as f:
        ...
        writer.writerow([
            ...
            "avg_acc_last_10_rounds",
            "avg_asr_last_10_rounds",
            "avg_selected_malicious_last_10_rounds",
            "avg_removed_benign_last_10_rounds"
        ])
