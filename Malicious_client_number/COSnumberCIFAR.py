...
# RS & LGP hyperparams
LGP_WARMUP_ROUNDS = 30       # warm-up 放宽层数门限
LGP_PASS_FRAC      = 0.6     # 通过需要的“诚实层占比”，warm-up 会自动放宽
...
def client_compute_trust_local_only(theta_prev_vec, delta_i_vec, prev_delta_i_vec=None,
    ...
    w_tda, w_rel, w_spk, w_w = 0.1205, 0.4374, 0.4421, 0.0000
    trust = (w_tda * tda01 +
             w_rel * rel_score +
             w_spk * spky_score +
             w_w   * wsign)
...
def one_round(round_idx, global_model, client_loaders, malicious_ids,
    ...
    # RS update: trusted + accepted_from_susp 两类用户都 +1
    # for cid in groups["trusted"]:
    #     RS[cid] = RS.get(cid, 0) + 1

    # for cid in groups["accepted_from_susp"]:
    #     RS[cid] = RS.get(cid, 0) + 1

    # # ----- RS summary & optional RS threshold -----
    # if len(RS) > 0:
    ...
    #     # always keep a copp as a default version
    #     groups["final_participants_afterRS"] = groups["final_participants"].copy()
    #
    #     # only apply RS threshold after warm-up rounds
    #     if APPLY_RS_HARD_THRESHOLD_IN_FINAL and round_idx > LGP_WARMUP_ROUNDS:
    #         before_rs = groups["final_participants"].copy() #记录筛选前的 final participants
    #         groups["final_participants_afterRS"] = [
    #             cid for cid in groups["final_participants"]
    #             if RS[cid] >= rs_thr  #应用RS门限
    #         ]
    #         after_rs = groups["final_participants_afterRS"]
    #         removed_by_rs = sorted(list(set(before_rs) - set(after_rs)))
    #         print(f"[RS Filter] Removed by RS threshold: {removed_by_rs}")
    #
    # # ----- Final aggregate: trusted + accepted suspicious (after optional RS cut) + RS selection -----
    # final_pool = [u for u in updates if u["cid"] in groups["final_participants_afterRS"]]
    # if len(final_pool) == 0: final_pool = trusted_updates74
    # agg_final, _ = weighted_average(final_pool)
    # final_global = copy.deepcopy(global_model); set_model_params_from_vector(final_global, global_vec + agg_final)
    #
    # # RS update: trusted + accepted_from_susp 两类用户都 +1
    ...
    # print(f"Final aggregation participants                        : {groups['final_participants_afterRS']}")
    print("\n=== Client Group Summary (this round) ===")
    print(f"Trusted (Top {int(split[0]*100)}%, OTA aggregated)        : {groups['trusted']}")
    print(f"Suspicious (middle {int(split[1]*100)}%, individual access): {groups['suspicious']}")
    print(f"Rejected initially (trust phase)                      : {groups['rejected']}")
...
def simulate_many_rounds(num_rounds=10, split=(0.50,0.30,0.20), verbose_round=True,
    ...
    for r in range(1, num_rounds+1):
        print(f"\n========== Round {r} | num_malicious = {num_malicious} ==========")
...
if __name__ == "__main__":
    ...
    malicious_num_list = [2, 4, 6, 8, 10, 12, 14, 16, 18]

    all_round_rows = []
    summary_rows = []

    for num_mal in malicious_num_list:
        print("\n" + "=" * 80)
        print(f"Running experiment for num_malicious = {num_mal}")
        print("=" * 80)
        ...
        results = simulate_many_rounds(
            num_rounds=100,   # 先和你当前程序一致；正式实验可改成 100
            split=(0.5, 0.3, 0.2),
            verbose_round=True,
        ...
        for i in range(len(LOGS["round"])):
            all_round_rows.append([
                ...
                json.dumps(LOGS["final_participants_ids"][i]),
                json.dumps(LOGS["denied_overall_ids"][i]),
                json.dumps(LOGS["removed_by_rs_ids"][i]),
                json.dumps(LOGS["selected_malicious_ids"][i]),
                json.dumps(LOGS["removed_benign_ids"][i]),
            ])

        final_acc_last = LOGS["acc_final"][-1]
        final_asr_last = LOGS["asr_final"][-1]
        avg_acc_last10 = float(np.mean(LOGS["acc_final"][-min(10, len(LOGS["acc_final"])):]))
        avg_asr_last10 = float(np.mean(LOGS["asr_final"][-min(10, len(LOGS["asr_final"])):]))
        avg_selected_mal_last10 = float(np.mean(LOGS["num_selected_malicious"][-min(10, len(LOGS["num_selected_malicious"])):]))
        avg_removed_benign_last10 = float(np.mean(LOGS["num_removed_benign"][-min(10, len(LOGS["num_removed_benign"])):]))
        ...
        summary_rows.append([
            ...
            avg_acc_last10,
            avg_asr_last10,
            avg_selected_mal_last10,
            avg_removed_benign_last10
        ])

        print("\n=== Summary for this malicious count ===")
        print(f"num_malicious = {num_mal}")
        print(f"malicious_ids = {MAL_IDS}")
        print(f"Last round   | Acc = {final_acc_last:.2f}% | ASR = {final_asr_last:.2f}%")
        print(f"Last-mean    | Acc = {avg_acc_last10:.2f}% | ASR = {avg_asr_last10:.2f}%")

    # 保存每轮明细
    detail_csv_path = "COSnumber0.csv"
    ...
    print(f"\nSaved round detail metrics to: {detail_csv_path}")

    # 保存汇总
    summary_csv_path = "COSnumber1.csv"
    with open(summary_csv_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([
            "num_malicious",
            "malicious_ids",
            "final_acc_last_round",
            "final_asr_last_round",
            "avg_acc_last_rounds",
            "avg_asr_last_rounds",
            "avg_selected_malicious_last_rounds",
            "avg_removed_benign_last_rounds"
        ])
        writer.writerows(summary_rows)

    print(f"Saved sweep summary to: {summary_csv_path}")
