...
POISON_FRACTION  = 0.2
BACKDOOR_TARGET  = 0
LAMBDA_EUC       = 0.1 #bigger, hidden, diffcult to find
    ...
    for _ in range(epochs):
        for i, (x, y) in enumerate(all_batches):
            ...
            if malicious and i in poison_batches:
                x_poison = x.clone()
                for k in range(x_poison.shape[0]): x_poison[k] = add_box_trigger(x_poison[k])
            ...
            if malicious and lambda_euc > 0:
                local_vec = model_params_to_vector(model)
                euc = torch.sum((local_vec - global_vec) ** 2)
                loss = loss + (lambda_euc * euc) / local_vec.numel()
...
def client_compute_trust_local_only(theta_prev_vec, delta_i_vec, prev_delta_i_vec=None,
    ...
    w_tda, w_rel, w_spk, w_w = 0.1205, 0.4374, 0.4421, 0
    trust = (w_tda * tda01 +
             w_rel * rel_score +
             w_spk * spky_score +
             w_w   * wsign)
...
def init_global_model():
    ...
    def _init(mm):
        if isinstance(mm, (nn.Conv2d, nn.Linear)) and hasattr(mm, "weight"):
...
if __name__ == "__main__":
    ...
    # 3) 保存每轮明细 CSV
    # =========================
    detail_csv_path = "EUCnumber0.csv"
    ...
    # 4) 保存汇总 CSV
    # =========================
    summary_csv_path = "EUCnumber1.csv"
