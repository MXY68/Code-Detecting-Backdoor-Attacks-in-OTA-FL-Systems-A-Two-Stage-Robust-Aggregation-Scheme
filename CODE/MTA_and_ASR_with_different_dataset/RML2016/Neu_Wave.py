...
def one_round(round_idx, global_model, client_loaders, malicious_ids,
    ...
    rel_list   = [u["rel_l2"] for u in updates]
    spk_list   = [u["spiky"] for u in updates]
    res_list   = [u["resid_ratio"] for u in updates]
    ws_list    = [u["wsign"] for u in updates]
    tmp01_list = [0.5 if np.isnan(u["temporal"]) else (u["temporal"] + 1.0) / 2.0 for u in updates]
    ali_list   = [cos_sim(u["delta"], ref0) for u in updates]
